...
import { parseInvoiceHTML } from "./main.ts";
...
for (const fixtureFile of fixtureFiles) {
  test(`Parsing ${path.basename(fixtureFile)}`, async () => {
    ...
    const items = order.shipments.reduce(
      (acc, shipment) => acc.concat(shipment.items),
      [],
    );
    ...
    assert.strictEqual(
      itemSubtotal,
      order.subtotalCents,
      `Subtotal should match the sum of item prices, but got ${itemSubtotal} vs ${order.subtotalCents}`,
    );
